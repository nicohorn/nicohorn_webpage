...
export async function middleware(req: NextRequest) {
  ...
  if (
    ...
    !req.nextUrl.pathname.includes("blog_entry") &&
    !req.nextUrl.pathname.includes("cron")
  ) {
    if (!token)
      return new Response("Falta autenticarse loco", {
        status: 401,
        statusText: "Not authenticated",
      });
    if (token?.user.role !== "admin") {
      return new Response("No sos admin wachin, rajá de acá", {
        status: 403,
